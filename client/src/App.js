...
class App extends Component {
  ...
  watchMyBets = async () => {
    ...
    contract
      .NewBet({ _addr: accounts[0] }, { fromBlock: 0, toBlock: "latest" })
      .watch(function(err, result) {
        ...
        console.log(
          "New bet by " + result.args._addr + " of " + result.args._value
        );
      });
  ...
  makeBet = async () => {
    ...
    // Check the pot
    try {
      let tp = await contract.methods
        .bet(accounts[0])
        .send({ from: accounts[0], value: "5000000000000000" });
      console.log(tp);
    } catch (error) {
      // return (
      //   <SnackbarMessage
      ...
      <div className={classes.layout}>
        ...
        <main>
          ...
          >
            ...
            <Grid item md={4} align="center">
              <h2>Win Now!</h2>
              <p>Only {Math.floor(betTimeDiff / 60)} minutes to go.</p>
            </Grid>
            <Grid item md={4} align="center">
              <Button
                classes={{
          ...
          >
            <Grid item md={6} className={classes.mainFeaturedPostContent}>
              The aim of the game is to be the last person to put ETH into the
              pot - if you remain there for 30 minutes then you win the entire
              pot instantly. Ok - the real aim is to practice coding and
              thinking about smart contracts and developing (secure) DApps.
              Unless if you make millions - then share!
              <Divider className={classes.divider} />
